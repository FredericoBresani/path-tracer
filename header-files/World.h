...
PointLight traceLight(std::vector<Object*> &objects, std::vector<Light*> &lights, Ambient *ambient, int depth)
{
    Vec3D lightNormal, lightX, lightZ;
    for (int i = 0; i < lights.size(); i++)
    {
        if (lights[i]->isExtense() && lights[i]->getLightModel()->getObjectType() == 't') {
            Point3D A, B, C;
            std::vector<Point3D> vertices = lights[i]->getMeshControlPoints();
            A = vertices[0];
            B = vertices[1];
            C = vertices[2];
...
RGBColor trace(const Ray &ray, std::vector<Object*> &objects, Camera &camera, std::vector<Light*> lights, Ambient *ambient, int depth)
{
    ...
    if (hInfo->hit_object) {
        ...
        if(l->castShadows() && getShadows) {
            ...
            } else { // precisa achar um caminho para luz
                if (hInfo->transparent) {
                    color = (color + trace(Ray(hInfo->hit_location, hInfo->toLight), objects, camera, lights, ambient, depth - 1))/2.0;
                } else {
                    if (kd > 0) {
                        color = (color + trace(Ray(hInfo->hit_location, difuseDirection), objects, camera, lights, ambient, depth - 1))/2.0;
                    }
